...
let count = 0;
//When user will click on the Create to do, the input div will translateX. (CREATE TODO BUTTON)
function translate() {
	document.getElementById('createTodoText').classList.toggle('textBoxTransition');
}
...
// Now lets write what will happen when user click on SAVE button.
const runProgram = () => {

	localStorage.setItem(`${todoTextTitle.value}`, `${todoTextPara.value}`)
	console.log(localStorage.getItem)

	let todoCard = document.createElement('div')/*CREATED A NEW DIV IN VIEWALLTODO */
	todoCard.classList.add('d-flex', 'border-bottom', 'w-100', 'align-items-center', 'p-1')
	...
	divH1.innerHTML = todoTextTitle.value1
	...
	let deletebtn = document.createElement('button')/*CREATED DELETE BOTTON IN todoCard */
	deletebtn.innerHTML = 'Delete'
	deletebtn.classList.add('btn', 'delete', 'btn-danger', 'btn-sm', 'ms-auto', 'me-2')
	...
	deletebtn.addEventListener('click', () => {
		...
		if (deleteconfirm) {
			todoCard.innerHTML = ""
			todoCard.classList.remove('rounded', 'border-bottom', 'w-100', 'mb-1', 'align-items-center', 'p-1')
...
const userClickedSave = () => { /* CLICKED ON SAVE*/
...
crossbtn.addEventListener('click', () => { //Click on Cross button inputtodo
...
/* CLICKED ON VIEW ALL TODO BOTTON*/
